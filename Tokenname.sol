...
contract TestToken is ITestToken, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "TestToken: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "testname: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __TestToken_v2_init_unchained(_router);
    }

    function __testtoken_v2_init_unchained(address _router) internal initializer {
        _name = "testname";
        _symbol = "testname";
        _decimals = 9;
        ...
        __TestToken_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __TestToken_tiers_init() internal initializer {
    ...
    preventBlacklisted(_account, "TestToken: Selected account is in blacklist")
    {
        require(_account != address(0), "TestToken: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "TestToken: Invalid address");
        require(_accountsTier[_account] > 0, "TestToken: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "TestToken: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "TestToken: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "TestToken: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "testname: Owner address is blacklisted")
    preventBlacklisted(spender, "testname: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "testname: Address is blacklisted")
    preventBlacklisted(from, "testname: From address is blacklisted")
    preventBlacklisted(to, "testname: To address is blacklisted")
    {
    ...
    function migrate(address account, uint256 amount) external override {
        require(migration != address(0), "testname: Migration is not started");
        require(_msgSender() == migration, "testname: Not Allowed");
